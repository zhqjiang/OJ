...
impl M {
    ...
    fn update_k(&mut self) {
        ...
        let mut r = Vec::new();
        let mut c = Vec::new();
        for (i, _) in self.rows_sums.iter().enumerate() {
            r.push(i as u8);
        }
        for (i, _) in self.columns_sums.iter().enumerate() {
            c.push(i as u8);
        }
        ...
        for i in &row_cs {
            for j in &column_cs {
                ...
                for &row_idx in i {
                    for &col_idx in j {
                        if self.matrices[r[row_idx as usize] as usize][c[col_idx as usize] as usize] == 0 {
